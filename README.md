...
In order to proceed with installation for this project, two dependencies need to be installed:
...
One the necessary dependencies have been installed, simply clone this repository, and execute the following command to build the virtual environment:
...
indicates the application is accessible via `localhost:8080`, on the host machine.  Otherwise, if ssl is configured, then the application is accessible via `https://localhost:8585`, on the host machine.
