...
When using the web-interface, it is important to ensure the csv, or xml file(s) are properly formatted. Dataset(s) poorly formatted will cause corresponding python scripts to fail in creating respective json dataset representation(s). This will subsequently prevent dataset(s) from being stored in their correponding database tables.
