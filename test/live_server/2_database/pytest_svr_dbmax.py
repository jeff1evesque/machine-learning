...
following svr cases for both anonymous and, authenticated users will be tested:
...
def test_max_collections_anon(client, live_server):
    ...
    assert collection.query('collection--pytest-svr--' + str(i + 1), 'find_one')

    # drop all collections
    for i in range(max_collection):
        assert entity.remove_entity(0, 'collection--pytest-svr--' + str(i + 1))
...
def test_max_collections_auth(client, live_server):
    ...
    assert collection.query('collection--pytest-svr--' + str(i), 'find_one')
    ...
    # drop all collections
    for i in range(max_collection):
        assert entity.remove_entity(1, 'collection--pytest-svr--' + str(i + 1))
        assert collection.query(
            'collection--pytest-svr--' + str(i + 1),
            'drop_collection'
        )
